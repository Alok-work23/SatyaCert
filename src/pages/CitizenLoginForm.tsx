// CitizenLoginForm.tsx
import React, { useEffect } from 'react';
import { Loader2 } from 'lucide-react';
import { UserRole } from '../types';
import { api } from '../services/api';

interface CitizenLoginFormProps {
  loading: boolean;
  setLoading: (v: boolean) => void;
  setError: (v: string) => void;
  setMsg: (v: string) => void;
  onLogin: (role: UserRole, userData?: any) => void;
}

export const CitizenLoginForm: React.FC<CitizenLoginFormProps> = ({
  loading, setLoading, setError, setMsg, onLogin,
}) => {

  // On mount: check if we just returned from Google redirect
  useEffect(() => {
    const checkRedirect = async () => {
      setLoading(true);
      try {
        const user = await api.handleGoogleRedirectResult();
        if (user) {
          // ── CRITICAL: save role BEFORE calling onLogin ──
          // onAuthStateChanged fires first on reload and checks localStorage.
          // If role isn't saved yet, it won't redirect. Save it now.
          localStorage.setItem('satyacert_role', UserRole.USER);
          onLogin(UserRole.USER, user);
        }
      } catch (e: any) {
        if (e?.code && e.code !== 'auth/null-user') {
          setError('Google sign-in failed: ' + (e.message ?? 'Unknown error'));
        }
      } finally {
        setLoading(false);
      }
    };
    checkRedirect();
  }, []);

  const handleGoogleLogin = async () => {
    setLoading(true);
    setError('');
    setMsg('');
    try {
      await api.loginGoogleRedirect();
      // Page redirects to Google — nothing runs after this line
    } catch (e: any) {
      setError('Could not start Google sign-in. Please try again.');
      setLoading(false);
    }
  };

  return (
    <div className="space-y-6">
      <div className="bg-blue-50 p-4 rounded text-sm text-blue-800 border border-blue-100">
        Citizens authenticate via Google linked to DigiLocker.
      </div>

      <button
        onClick={handleGoogleLogin}
        disabled={loading}
        className="w-full flex items-center justify-center gap-3 bg-white text-gray-700
                   border border-gray-300 hover:bg-gray-50 font-bold py-3 px-4 rounded shadow-sm
                   disabled:opacity-60 disabled:cursor-not-allowed transition-colors"
      >
        {loading ? (
          <Loader2 className="animate-spin w-5 h-5" />
        ) : (
          <>
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Sign in with Google
          </>
        )}
      </button>
    </div>
  );
};